import pygame
pygame.font.init() 
from pygame.locals import *
from bala import *
#from personaje_principal import Personaje
from personaje_principal_copy import Personaje
from loads import *
from Modo import *
from premio import Premio
import random
from enemigo import Enemigo
from spawn import *
from moneda import *
from trampa import *
from vida import *
from plataformas import *
from plataforma_movible import *
import re
from nivel import *
from Boss import *
from SQLite import *

class Nivel:
    def __init__(self,fondo,pantalla, personaje_principal,lista_plataformas,plataformas_movibles, respawn, bala, lista_monedas, imagen_fondo, diccionario_animaciones, tiempo_ultimo_disparo, flag_disparo, duracion_maxima) -> None:
        self.pantalla = pantalla
        self.jugador = personaje_principal
        self.respawn = respawn
        self.bala = bala
        self.plataformas = lista_plataformas
        self.plataformas_movibles = plataformas_movibles
        self.lista_monedas = lista_monedas
        self.imagen_fondo = imagen_fondo
        self.diccionario_animaciones = diccionario_animaciones
        self.tiempo_ultimo_disparo = tiempo_ultimo_disparo
        self.flag_disparo = flag_disparo
        self.duracion_maxima = duracion_maxima
        # Tiempo inicial en milisegundos
        self.tiempo_inicial = pygame.time.get_ticks()
        self.imagen_fondo = fondo


        # Variable para verificar si el juego está en ejecución
        self.juego_en_ejecucion = True

    def update(self, eventos):
        for evento in eventos:
            if evento.type == QUIT:
                pygame.quit()
                sys.exit()
            elif evento.type == KEYDOWN:
                if evento.key == K_TAB:
                    cambiar_modo()
        self.leer_teclas()
        self.actualizar_pantalla()
        self.actualizar_respawn()
        self.actualizar_habilidad()
        self.dibujar_rectangulos()
        self.blit_texto()
        self.verificar_fin_juego()
            

    def leer_teclas(self):
        teclas = pygame.key.get_pressed()

        if teclas[pygame.K_d]:
            self.jugador.que_hace = "Derecha"
        elif teclas[pygame.K_a]:
            self.jugador.que_hace = "Izquierda"
        elif teclas[pygame.K_w]:
            self.jugador.que_hace = "Salta"
        else:
            self.jugador.que_hace = "Quieto"

        self.flag_disparo = False
        if teclas[pygame.K_f]:
            self.flag_disparo = True
    
        
    def dibujar_rectangulos(self):
        if obtener_modo():
            pygame.draw.rect(self.pantalla, "blue", self.jugador.rectangulo_principal, 3)

            for plataforma in self.plataformas:
                pygame.draw.rect(self.pantalla, 'red', plataforma['rectangulo'], 3)

    def actualizar_pantalla(self):
        self.pantalla.blit(self.imagen_fondo, (0, 0))
        for trampa in self.plataformas_movibles:
            trampa.actualizar(self.pantalla)
            self.jugador.manejar_colision_trampa(trampa)
        self.jugador.actualizar(self.pantalla, self.plataformas)
        self.respawn.generar_todo(self.diccionario_animaciones)
        for enemigo in self.respawn.lista_enemigos:
            if isinstance(enemigo, Boss):  # Verificar si es el nuevo jefe
                enemigo.actualizar(self.pantalla, self.plataformas, self.jugador)
            else:
                enemigo.actualizar(self.pantalla, self.plataformas, self.jugador)
        

    def obtener_tiempo(self):
        return pygame.time.get_ticks()
    
    def actualizar_respawn(self):
        for moneda in self.lista_monedas:
            moneda.actualizar(self.pantalla)
            self.jugador.obtener_moneda(self.lista_monedas)

        for vida in self.respawn.lista_vidas:
            vida.actualizar(5,self.plataformas,self.pantalla)
            self.respawn.sumar_vida()

        for premio in self.respawn.lista_premios:
            if not premio.tocada:
                premio.actualizar(5,self.plataformas,self.pantalla)
                #self.pantalla.blit(premio.image, premio.rect)
                #premio.disponible = True
            # Procesar el nuevo premio solo si aún no ha sido tocado
            if premio.disponible: 
                self.jugador.verificar_colision_premio(premio)

        for enemig in self.respawn.lista_enemigos:
            if not enemig.esta_muerto:
                # Actualizar enemigo
                enemig.actualizar(self.pantalla, self.plataformas, self.jugador)
                # Verificar colisiones con el jugador
                self.respawn.verificar_colision_enemigo_respawn(enemig, self.pantalla)

    def actualizar_habilidad(self):
        if self.jugador.habilidad_especial and self.flag_disparo:
            tiempo_actual = pygame.time.get_ticks()
            if tiempo_actual - self.tiempo_ultimo_disparo >= 1000:
                self.jugador.lanzar_proyectiles(self.bala)
                self.tiempo_ultimo_disparo = tiempo_actual
        self.bala.actualizar_proyectiles(self.pantalla,self.plataformas)

    def blit_texto(self):
        tiempo_actual = self.obtener_tiempo()
        tiempo_transcurrido = (tiempo_actual - self.tiempo_inicial) // 1000  # Convertir a segundos
        tiempo_restante = max(0, self.duracion_maxima - tiempo_transcurrido * 1000)
        minutos = tiempo_restante // 60000  # Convertir milisegundos a minutos
        segundos = (tiempo_restante % 60000) // 1000  # Obtener los segundos restantes
        tiempo_formateado = f'{minutos:02}:{segundos:02}'

        font = pygame.font.Font(None, 36)
        texto_superficie = font.render(tiempo_formateado, True, (255, 255, 255))
        self.pantalla.blit(texto_superficie, (483, 19))

        font = pygame.font.Font(None, 36)
        texto_superficie = font.render(tiempo_formateado, True, (255, 255, 255))
        self.pantalla.blit(texto_superficie, (483, 19))
        # Blitear la cantidad de vidas
        font = pygame.font.Font(None, 36) 
        texto_vidas = font.render(f"X {self.jugador.vidas}", True, (255, 255, 255))
        self.pantalla.blit(texto_vidas, (123, 36))

        # Blitear la cantidad de puntos
        texto_puntos = font.render(f"Puntos: {self.jugador.puntuacion}", True, (255, 255, 255))
        self.pantalla.blit(texto_puntos, (815, 30))

    def verificar_fin_juego(self):
        tiempo_transcurrido_actual = pygame.time.get_ticks()
        if tiempo_transcurrido_actual >= self.duracion_maxima:
            if self.jugador.esta_muerto:
                self.jugador.puntuacion = 0
            nombre_jugador = "Jugador1"  # Reemplazar con el nombre del jugador (puedes obtenerlo de alguna manera en tu juego)
            puntuacion_jugador = self.jugador.puntuacion  # Reemplazar con la puntuación actual del jugador
            guardar_puntuacion(nombre_jugador, puntuacion_jugador)
            finalizo_game.play()
            print("Fin del juego")
